...
def main():
    ...
    # Transformation for awards
    def awards_transformation(awards_df):
        """
        Transforms the awards DataFrame by converting the 'year' column to an integer type.

        Parameters:
        awards_df (DataFrame): A pandas DataFrame containing award information, including a 'year' column.

        Returns:
        DataFrame: Transformed DataFrame with the 'year' column as an integer type.
        """
    ...
    if not df.empty:
        ...
        transformed_df = awards_transformation(basic_transformed_df)
